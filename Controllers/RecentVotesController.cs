...
namespace GvGRank_Server.Controllers
{
	...
	public class RecentVotesController : ControllerBase
	{
		...
		public string[][] Get(int count = 1, string player = "")
		{
			...
			// If a valid player name is provided
			if (!string.IsNullOrEmpty(player) && _context.Players.Where(x => x.Name == player).Any())
				result = result.Where(p => p.v2.p1.Name == player || p.p2.Name == player);
